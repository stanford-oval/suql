...
```
   - If you wish to choose another credential, change the default values for `user`, `password`, and `database` there to match your PostgreSQL set up. This user also appears once in `src/suql/sql_free_text_support/execute_free_text_sql.py` of the SUQL compiler, so that intermediate tables created can be queried. Please also change it to match your user name.
