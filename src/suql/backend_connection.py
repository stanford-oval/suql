...
class BackendConnection:
    def __init__(
        self, greeting="Hi! How can I help you?", engine="text-davinci-003"
    ) -> None:
        client = MongoClient(CONNECTION_STRING)
        self.db = client["yelpbot"]  # the database name is yelpbot
        self.table = self.db[
            "dialog_turns_studies"
        ]  # the collection that stores dialog turns
    ...
    def compute_next(
        self, dialog_id, user_utterance, turn_id, system_name, experiment_id
    ) -> DialogueTurn:
        tuples = list(
            self.table.find({"dialogID": dialog_id}).sort("turn_id", ASCENDING)
        )
        ...
        dlgHistory = compute_next_turn(
            dlgHistory, user_utterance, engine=self.engine, sys_type=system_name
        )

        # update the current tuple with new DialogTurn
        new_tuple = {
            "_id": "(" + str(dialog_id) + ", " + str(turn_id) + ")",
            "dialogID": dialog_id,
            "turn_id": turn_id,
            "created_at": datetime.utcnow(),
            "dlg_turn": dlgHistory[-1].__dict__,
...
def chat():
    ...
    if user_utterance == "FINISHED":
        return {
            "agent_utterance": f"{dialog_id} is your dialog ID. Please submit it in the Google Form.",
            "log_object": {},
        }

    dlgItem = connection.compute_next(
        dialog_id, user_utterance, turn_id, system_name, experiment_id
    )
    ...
    log["db_results"] = (
        json.loads(dlgItem.genie_utterance)
        if dlgItem.genie_utterance is not None
        else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ssl_key_file", type=str, help="Where to read the SSL certificate for HTTPS"
    )
