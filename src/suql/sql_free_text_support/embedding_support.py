...
from suql.reviews_server import _compute_single_embedding, compute_sha256
...
class EmbeddingStore:
    def __init__(
        self, table_name, primary_key_field_name, free_text_field_name
    ) -> None:
        # stores three lists:
        ...
        self.initialize_from_sql(
            table_name, primary_key_field_name, free_text_field_name
        )
    ...
    def initialize_embedding(self):
        ...
        for i, document in tqdm(list(enumerate(self.all_free_text))):
            ...
            if current_counter == 0:
                ...
                existing_embeddings = torch.cat(
                    (existing_embeddings, document_embeddings), dim=0
                )

            # stores the bidirectional mapping
            self.document2embedding[i] = [
                num
                for num in range(
                    current_counter, current_counter + document_embeddings.size(0)
                )
            ]
            for num in range(
                current_counter, current_counter + document_embeddings.size(0)
        ...
        if memory_bytes < 1024:
            ...
            print(
                f"embedding initialized and now using {memory_bytes / (1024 ** 2):.2f} MB"
            )
        else:
            print(
                f"embedding initialized and now using {memory_bytes / (1024 ** 3):.2f} GB"
            )
    ...
    def dot_product(self, id_list, query, top):
        ...
        dot_products = torch.sum(
            self.embeddings[torch.tensor(embedding_indices, device="cuda")]
            * query_embedding,
            dim=1,
        )
        ...
        embeddings_indices_max = [
            embedding_indices[index] for index in indices_max.tolist()
        ]
        return [
            (
                self.document2id[self.embedding2document[index]],
                [self.all_free_text[self.embedding2document[index]]],
            )
            for index in embeddings_indices_max
        ]
    ...
    def dot_product_with_value(self, id_list, query):
        ...
        for id in id_list:
            ...
            id_embedding_indices = [
                item
                for sublist in map(
                    lambda x: self.document2embedding[x], self.id2document[id]
                )
                for item in sublist
            ]
            ...
            if not id_embedding_indices:
                ...
                top_value, top_index = torch.topk(
                    dot_products[
                        torch.tensor(
                            list(range(counter, counter + len(id_embedding_indices)))
                        )
                    ],
                    1,
                )
                # getting the actual document requires going from top_index to actual embedding index to document index
                top_document = self.all_free_text[
                    self.embedding2document[id_embedding_indices[top_index]]
                ]
...
class MultipleEmbeddingStore:
    ...
    def dot_product(self, table_name, id_list, field_query_list, top):
        ...
        if len(field_query_list) == 1:
            ...
            return self.retrieve(table_name, free_text_field).dot_product(
                id_list, query, top
            )

        res = {}
        for free_text_field, query in field_query_list:
            for id, top_value, top_document in self.retrieve(
                table_name, free_text_field
...
def search():
    ...
    res = {
        "result": embedding_store.dot_product(
            data["table_name"], data["id_list"], data["field_query_list"], data["top"]
        )
    }
