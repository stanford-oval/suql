...
def verify_single_res(doc, field_query_list):
    ...
    for i, entry in enumerate(field_query_list):
        ...
        if not verify(doc[i], field, query, operator, value):
            ...
            found_stmt.append(
                "Verified answer({}, '{}') {} {} based on document: {}".format(
                    field, query, operator, value, doc[i]
                )
            )
    if all_found:
        print("\n".join(found_stmt))
...
def retrieve_and_verify(
    ...
    id_index = list(map(lambda x: x[0], column_info)).index(
        "_id"
    )  # TODO: this is hard-coded for restaurants. Automate fetching this

    # get _id list from `existing_results`
    start_time = time.time()
    data = {
        "table_name": node.fromClause[
            0
        ].relname,  # TODO: implement support for JOIN statements
        "id_list": list(map(lambda x: x[id_index], existing_results)),
        "field_query_list": list(map(lambda x: (x[0], x[1]), field_query_list)),
        "top": limit
        * 20,  # return 20 times the ordered amount, for GPT filtering purposes, TODO: this needs to be better planned
    }

    # Send a POST request
    response = requests.post(
        "http://127.0.0.1:8509/search",
        json=data,
    ...
    if parallel:
        ...
        id_res = parallel_filtering(
            lambda x: verify_single_res(x[1], field_query_list),
            parsed_result,
            limit,
    ...
    def visit_A_Expr(self, ancestors: Ancestor, node: A_Expr):
        ...
        try:
            res, column_infos = execute_sql_with_column_info(
                RawStream()(to_execute_node), unprotected=True
            )
...
def execute_free_text_queries(
    node, predicate: BoolExpr, existing_results, column_info, limit
):
    ...
    if isinstance(predicate, A_Expr):
        ...
        return (
            retrieve_and_verify(
                node, field_query_list, existing_results, column_info, limit
            ),
            column_info,
        )
        ...
        raise ValueError(
            "expects predicate to only contain automatic unstructural query or AND of them, but predicate is not: {}".format(
                RawStream()(predicate)
            )
        )
...
def execute_and(sql_dnf_predicates, node: SelectStmt, limit, cache: dict):
    ...
    if (
        ...
        structural_res, column_info = execute_structural_sql(
            node, structural_predicates, cache
        )
        ...
        return execute_free_text_queries(
            node, free_text_predicates, structural_res, column_info, limit
        )
        ...
        if if_all_structural(sql_dnf_predicates):
            ...
            return execute_free_text_queries(
                node, sql_dnf_predicates, all_results, column_info, limit
            )
...
if __name__ == "__main__":
    ...
    root = parse_sql(
        "SELECT opening_hours->>'Thursday' FROM restaurants WHERE name ILIKE 'Top of the Mark' LIMIT 1;"
    )  # FIXIT
