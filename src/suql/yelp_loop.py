...
from suql.prompt_continuation import batch_llm_generate, llm_generate

# from query_reviews import review_server_address
from suql.sql_free_text_support.execute_free_text_sql import SelectVisitor
from suql.utils import (
    handle_opening_hours,
    if_usable_restaurants,
    input_user,
...
def parse_execute_sql(dlgHistory, user_query, prompt_file="prompts/parser_sql.prompt"):
    first_sql, first_sql_time = llm_generate(
    ...
    try:
        ...
        print(
            "intermediate (temp) SUQL query executed at the end: {}".format(second_sql)
        )
    ...
    return (
        ...
        second_sql,
        first_sql_time,
        second_sql_end_time - second_sql_start_time,
        cache,
    )
    ...
    if continuation.startswith("Yes"):
        if sys_type == "sql_textfcns_v0801":
            (
                ...
                dlgHistory, user_utterance, prompt_file="prompts/parser_sql.prompt"
            )
            ...
            (
                ...
                dlgHistory,
                user_utterance,
                prompt_file="prompts/parser_sql_semantic_index.prompt",
            )
            dlgHistory[-1].genie_utterance = json.dumps(results, indent=4)
            dlgHistory[-1].user_target = first_sql
            dlgHistory[-1].temp_target = second_sql
            ...
            from src.suql.reviews_server import baseline_filter
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file", type=str, default="log.log", help="Where to write the outputs."
    )
    parser.add_argument(
        "--engine",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Write results in TSV format to file",
    )
    parser.add_argument(
        "--batch_process", type=str, default=None, help="A list of QA inputs to run"
    )
    ...
    if args.batch_process:
        ...
        for each_input in inputs:
            ...
            dlgHistory = compute_next_turn(
                dlgHistory, utterance, engine=args.engine, sys_type=args.sys_type
            )
            with open(args.record_result, "a+") as fd:
                fd.write(
                    "{}\t{}\t{}\t{}\t{}\n".format(
                        id,
                        dlgHistory[-1].user_utterance,
                        dlgHistory[-1].user_target,
                        dlgHistory[-1].agent_utterance,
                        "\t".join(dlgHistory[-1].db_results),
                    )
                )
    ...
    try:
        while True:
            ...
            dlgHistory = compute_next_turn(
                dlgHistory, user_utterance, engine=args.engine, sys_type=args.sys_type
            )
